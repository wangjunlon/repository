 ...
 * 
 * 实现全局唯一的ObjectId
 * getObjectId 生成一个16进制的ObjectId
 * get 生成一个ObjectId
 * isValid 判断字符串是否为ObjectId
 * createFromLegacyFormat 使用毫秒数、机器码和增量值创建一个ObjectId
 * toByteArray 转换为byte
 * toHexString 转换为16进制
 * equals 比较
 * @author liujia
...
public final class ObjectId implements Comparable<ObjectId>, Serializable {
    ...
    //生成数字型ID
    private static final IdGenerator IDG = IdGenerator.INSTANCE;
     ...
     * @param counter the counter
     * @throws IllegalArgumentException if the high order byte of
     *             machineIdentifier or counter is not zero
     */
